...
bxh_files=`find /data/qa_queue | grep ".bxh$"`
...
TR=`grep "Repetition" $PARFILE | awk -F ':' '{print $2}' | tr -d '[[:space:]]'`
	...
	echo $trline | sed "s#<spacing>0</spacing>#<spacing>2000</spacing>#"
	echo $trline | sed "s#<spacing>0</spacing>#<spacing>2000</spacing>#" >> $file.new
	...
	echo /usr/sbin/fBIRN/fmriqa_phantom.pl --overwrite $file $outputdir |ts >> $logfile
	/usr/sbin/fBIRN/fmriqa_phantom.pl --overwrite $file $outputdir |ts >> $logfile
    else
	echo /usr/sbin/fBIRN/fmriqa_generate2.pl --overwrite --indexnonjs $file $outputdir |ts >> $logfile
